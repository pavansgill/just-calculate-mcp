...
pub struct TrigInput {
    #[schemars(description = "Operation: sin, cos, tan, sec, csc, cot, asin, acos, atan, asec, acsc, acot")]
    pub operation: String,
    #[schemars(description = "Input angle (for forward ops) or ratio (for inverse ops)")]
    pub value: f64,
    #[schemars(description = "Angle unit for forward operations: degrees or radians (default radians)")]
...
pub fn trigonometry_2arg(input: Trig2ArgInput) -> String {
    match input.operation.as_str() {
        "atan2" => {
            ...
            format!("atan2({}, {}) = {} radians = {} degrees", input.y, input.x, r, rad_to_deg(r))
        }
        "hypot" => format!("hypot({}, {}) = {}", input.y, input.x, input.y.hypot(input.x)),
...
pub struct HyperbolicInput {
    #[schemars(description = "Operation: sinh, cosh, tanh, sech, csch, coth, asinh, acosh, atanh, asech, acsch, acoth")]
    pub operation: String,
...
pub struct AngleConvertInput {
    #[schemars(description = "Operation: deg_to_rad, rad_to_deg, deg_to_grad, grad_to_deg, rad_to_grad, grad_to_rad, normalize_360, normalize_180")]
    pub operation: String,
...
mod tests {
    ...
    fn trig(v: f64, op: &str, unit: Option<&str>) -> String {
        trigonometry(TrigInput { operation: op.to_string(), value: v, angle_unit: unit.map(|s| s.to_string()) })
    }
    ...
    #[test]
    fn test_cos_0() { assert!(trig(0.0, "cos", None).contains("1")); }
    #[test]
    fn test_asin_domain_err() { assert!(trig(2.0, "asin", None).contains("Error")); }
    #[test]
    fn test_hypot() {
        let r = trigonometry_2arg(Trig2ArgInput { operation: "hypot".to_string(), y: 3.0, x: 4.0 });
        assert!(r.contains("5"), "{r}");
    }
    #[test]
    fn test_sinh_zero() { assert!(hyperbolic(HyperbolicInput { operation: "sinh".to_string(), value: 0.0 }).contains("0")); }
    #[test]
    fn test_atanh_invalid() { assert!(hyperbolic(HyperbolicInput { operation: "atanh".to_string(), value: 1.0 }).contains("Error")); }
    #[test]
    fn test_deg_to_rad() {
        let r = angle_convert(AngleConvertInput { operation: "deg_to_rad".to_string(), value: 180.0 });
        assert!(r.contains("3.14"), "{r}");
    }
    #[test]
    fn test_normalize_360() {
        let r = angle_convert(AngleConvertInput { operation: "normalize_360".to_string(), value: -90.0 });
