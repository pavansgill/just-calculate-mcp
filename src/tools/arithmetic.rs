...
pub struct RoundingInput {
    ...
    #[schemars(description = "Decimal places (round_dp) or significant figures (round_sf). Ignored for other operations.")]
...
mod tests {
    ...
    fn rnd(v: f64, op: &str, p: Option<u32>) -> String {
        round(RoundingInput { value: v, operation: op.to_string(), precision: p })
    }

    #[test]
    fn test_floor() { assert_eq!(rnd(3.7, "floor", None), "floor(3.7) = 3"); }
    #[test]
    fn test_ceil() { assert_eq!(rnd(3.2, "ceil", None), "ceil(3.2) = 4"); }
    #[test]
    fn test_round() { assert_eq!(rnd(3.5, "round", None), "round(3.5) = 4"); }
    #[test]
    fn test_truncate() { assert_eq!(rnd(-3.9, "truncate", None), "truncate(-3.9) = -3"); }
    #[test]
    fn test_round_dp() { assert!(rnd(3.14159, "round_dp", Some(2)).contains("3.14")); }

    fn prop(v: f64, op: &str, b: Option<f64>, c: Option<f64>) -> String {
        number_properties(NumberPropertiesInput { operation: op.to_string(), value: v, value_b: b, value_c: c })
    }

    #[test]
    fn test_abs() { assert_eq!(prop(-5.0, "abs", None, None), "|-5| = 5"); }
    #[test]
    fn test_sign() { assert!(prop(-3.0, "sign", None, None).contains("-1")); }
    #[test]
    fn test_reciprocal() { assert!(prop(4.0, "reciprocal", None, None).contains("0.25")); }
    #[test]
    fn test_min() { assert!(prop(3.0, "min", Some(7.0), None).contains("3")); }
    #[test]
    fn test_max() { assert!(prop(3.0, "max", Some(7.0), None).contains("7")); }
    #[test]
    fn test_clamp() { assert!(prop(10.0, "clamp", Some(0.0), Some(5.0)).contains("5")); }
    #[test]
    fn test_gcd() { assert!(prop(12.0, "gcd", Some(8.0), None).contains("4")); }
    #[test]
    fn test_lcm() { assert!(prop(4.0, "lcm", Some(6.0), None).contains("12")); }
}
