...
pub fn math_constant(input: MathConstantInput) -> String {
    match input.name.to_lowercase().as_str() {
        ...
        "c" | "speed_of_light" => format!("c = 299792458 m/s (speed of light in vacuum)"),
        "g" | "gravity" | "gravitational_acceleration" => {
            format!("g = 9.80665 m/s² (standard gravitational acceleration)")
        }
        "h" | "planck" => format!("h = 6.62607015e-34 J·s (Planck constant)"),
        "avogadro" | "na" => format!("Nₐ = 6.02214076e23 mol⁻¹ (Avogadro constant)"),
        "boltzmann" | "kb" | "k_b" => {
            format!("kB = 1.380649e-23 J/K (Boltzmann constant)")
        }
