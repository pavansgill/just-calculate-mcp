...
fn next_prime_after(n: u64) -> u64 {
    ...
    while !is_prime_impl(candidate) { candidate += 1; }
    candidate
}
...
pub struct NumberTheoryInput {
    #[schemars(description = "Operation: gcd, lcm, is_prime, prime_factors, next_prime, nth_prime, divisors, divisor_count, sum_of_divisors, euler_totient, fibonacci, lucas, digit_sum, digital_root")]
...
pub fn number_theory(input: NumberTheoryInput) -> String {
    ...
    match input.operation.as_str() {
        "gcd" => {
            let m = match input.m { Some(x) => x, None => return "Error: gcd requires m".to_string() };
            format!("gcd({n}, {m}) = {}", gcd(n, m))
        }
        "lcm" => {
            let m = match input.m { Some(x) => x, None => return "Error: lcm requires m".to_string() };
            format!("lcm({n}, {m}) = {}", lcm(n, m))
        }
        "is_prime" => format!("{n} is{} prime", if is_prime_impl(n) { "" } else { " not" }),
        "prime_factors" => {
            let f = prime_factors_impl(n);
            format!("prime_factors({n}) = [{}]", f.iter().map(|x| x.to_string()).collect::<Vec<_>>().join(", "))
        }
        "next_prime" => format!("next_prime after {n} = {}", next_prime_after(n)),
        "nth_prime" => {
            if n == 0 { return "Error: nth_prime is 1-indexed".to_string(); }
            if n > 10_000 { return "Error: n too large (max 10,000)".to_string(); }
            let mut count = 0u64;
            let mut candidate = 1u64;
            loop {
                candidate += 1;
                if is_prime_impl(candidate) { count += 1; }
                if count == n { break; }
            }
        ...
        "divisors" => {
            ...
            divs.sort();
            format!("divisors({n}) = [{}]", divs.iter().map(|x| x.to_string()).collect::<Vec<_>>().join(", "))
        }
        "divisor_count" => {
        ...
        "fibonacci" => {
            if n > 93 { return "Error: fibonacci({n}) overflows u64; max is 93".to_string(); }
            let (mut a, mut b) = (0u64, 1u64);
            for _ in 0..n { let t = a + b; a = b; b = t; }
            format!("fibonacci({n}) = {a}")
        }
        "lucas" => {
            if n > 92 { return "Error: lucas({n}) overflows u64; max is 92".to_string(); }
...
pub struct ModularArithmeticInput {
    ...
    pub a: u64,
    #[schemars(description = "Exponent (mod_pow) or second operand (mod_inverse: unused, mod_add/mod_multiply: b)")]
    pub b: u64,
...
fn mod_pow_impl(mut base: u64, mut exp: u64, modulus: u64) -> u64 {
    ...
    base %= modulus;
    while exp > 0 {
        if exp % 2 == 1 { result = (result as u128 * base as u128 % modulus as u128) as u64; }
        exp >>= 1;
...
pub fn modular_arithmetic(input: ModularArithmeticInput) -> String {
    ...
    if m == 0 { return "Error: modulus must be > 0".to_string(); }
...
mod tests {
    ...
    fn nt(n: u64, op: &str, m: Option<u64>) -> String {
        number_theory(NumberTheoryInput { operation: op.to_string(), n, m })
    }

    #[test]
    fn test_gcd() { assert!(nt(12, "gcd", Some(8)).contains("4")); }
    #[test]
    fn test_is_prime_true() { assert!(nt(7, "is_prime", None).contains("is prime")); }
    #[test]
    fn test_is_prime_false() { assert!(nt(9, "is_prime", None).contains("is not prime")); }
    #[test]
    fn test_fibonacci() { assert!(nt(10, "fibonacci", None).contains("55")); }
    #[test]
    fn test_lucas() { assert!(nt(5, "lucas", None).contains("11")); }
    #[test]
    fn test_digit_sum() { assert!(nt(123, "digit_sum", None).contains("6")); }
    #[test]
    fn test_digital_root() { assert!(nt(493, "digital_root", None).contains("7")); }
    #[test]
    fn test_euler_totient() { assert!(nt(9, "euler_totient", None).contains("6")); }
    #[test]
    fn test_mod_pow() {
        let r = modular_arithmetic(ModularArithmeticInput { operation: "mod_pow".to_string(), a: 2, b: 10, modulus: 1000 });
        assert!(r.contains("24"), "{r}");
    }
    #[test]
    fn test_mod_inverse() {
        let r = modular_arithmetic(ModularArithmeticInput { operation: "mod_inverse".to_string(), a: 3, b: 0, modulus: 7 });
